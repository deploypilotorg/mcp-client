...
class MCPHttpClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.server_url}/initialize") as response:
                if response.status != 200:
                    ...
                    raise ConnectionError(
                        f"Failed to initialize connection: {error_text}"
                    )
            ...
            async with self.session.get(f"{self.server_url}/list_tools") as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise ConnectionError(f"Failed to list tools: {error_text}")
    ...
    async def call_tool(
        ...
        try:
            # Prepare the request
            request_data = {"name": tool_name, "arguments": arguments}
            ...
            async with self.session.post(
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise ConnectionError(f"Failed to execute tool: {error_text}")
