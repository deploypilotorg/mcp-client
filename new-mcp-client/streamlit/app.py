    ...
    try:
        async with get_aiohttp_session() as session:
            ...
            async with session.post(
                f"{st.session_state.server_url}/execute_tool", 
                json=payload
            ) as response:
                if response.status == 200:
                    data = await response.json()
...
"""
    ...
    try:
        ...
        try:
            ...
            if tool_name:
                # Execute the tool
                success, result = await execute_tool(tool_name, arguments)
                    ...
                    if tool_name == "github_repo" and arguments.get("action") == "clone":
                        ...
                        for line in response_lines:
                            if "to " in line and "repo_path" not in st.session_state:
                                ...
                                st.session_state.repo_path = path_part
                    
                    return result
                else:
                    return f"Error executing tool: {result}"
            else:
                return response
        except Exception as e:
...
with st.sidebar:
    ...
    server_url_input = st.text_input(
        "Server URL", 
        value=DEFAULT_MCP_SERVER_URL if not st.session_state.server_url else st.session_state.server_url,
    ...
    if st.button("Clone Repository"):
        if st.session_state.is_connected:
            with st.spinner("Cloning repository..."):
                success, result = asyncio.run(execute_tool(
                    "github_repo", 
                    {
                        "action": "clone",
                        "repo_url": repo_url
    ...
    if st.session_state.repo_path:
        ...
        if analysis_action == "search_code":
            search_query = st.text_input("Search Query", placeholder="Enter search pattern")
        ...
        if deploy_action == "prepare_deployment":
            deploy_type = st.selectbox(
                "Deployment Type",
                options=["static", "docker", "heroku", "custom"]
            )
            ...
            if deploy_type == "static":
                deploy_config["build_dir"] = st.text_input("Build Directory", value="build")
                deploy_config["build_command"] = st.text_input("Build Command", value="npm run build")
                deploy_config["deploy_target"] = st.text_input("Deploy Target", value="/var/www/html")
                deploy_config["create_if_missing"] = st.checkbox("Create if missing", value=True)
            
            elif deploy_type == "docker":
                deploy_config["dockerfile_path"] = st.text_input("Dockerfile Path", value="Dockerfile")
                deploy_config["image_name"] = st.text_input("Image Name", value="myapp")
                deploy_config["container_name"] = st.text_input("Container Name", value="myapp-container")
                ...
                deploy_config["script_path"] = st.text_input("Script Path", value="deploy.sh")
